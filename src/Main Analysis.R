...
###### 3.2.1. Define the indicator columns and the weighting variable ######
...
###### 3.2.2. Collapse to region–year–country level ######
# Because indicators are country-level, we first aggregate all articles
# from the same country, in the same region, and same year.

data_country_year_region <- data_filtered %>%
  # Drop rows with missing country or region (cannot be grouped properly)
  filter(!is.na(country), !is.na(region)) %>%
  group_by(region, PY, country) %>%
  summarise(
    # For each country-year-region, take the mean of all indicators
    # (in case there are multiple articles from the same country-year)
    across(
      all_of(c(indicator_cols, weight_var)),
      ~ mean(.x, na.rm = TRUE)
...
###### 3.2.3. Compute population-weighted averages by region–year ######
# Now we aggregate from country-level to region-level for each year,
# using "Population in year" as the weight.

indicators_region <- data_country_year_region %>%
  group_by(region, PY) %>%
  summarise(
    # For each indicator, compute the population-weighted mean
    across(
    ...
    # Also keep the total population for that region–year
    `Population in year` = sum(`Population in year`, na.rm = TRUE),
    .groups = "drop"
  )


###### 3.2.4. Original publication-level summary (counts, citations, quartiles) ######

data_filtered_region <- data_filtered %>%
  group_by(region, PY) %>%
  summarise(
    # Number of publications in that region–year
    total_publications = n(),
    # Total citations in that region–year
    citations = sum(TC, na.rm = TRUE),
    # Number of papers in each quartile
    Q1 = sum(Cuartil == "Q1", na.rm = TRUE),
    Q2 = sum(Cuartil == "Q2", na.rm = TRUE),
  ...
  # 5. Join the publication summary with the weighted indicators
  left_join(indicators_region, by = c("region", "PY"))
...
# Region models
results.regressions.region <- run_two_var_models_by_group(
  ...
  dplyr::rename(region = group_name)
...
#results.hierarchical.region = read_excel("~/Desktop/food-security/data/region/Supplementary Material 2.xlsx")

results.hierarchical.region <- run_hierarchical_models(
  data = data_filtered_region,
  group_var = "region",
  indicator_roles = indicator_roles,
  pub_var = "total_publications",
...
#results.moderator_screening.region = read_excel("~/Desktop/food-security/data/region/Supplementary Material 3.xlsx")

results.moderator_screening.region <- run_moderator_screening_mixed(
  data = data_filtered_region,
  group_var = "region",
  indicator_roles = indicator_roles,
  indicator_clusters = indicator_clusters,
...
summary_region <- results.moderator_screening.region %>%
  summarise(
    n_tests = n(),
    n_failed = sum(!is.na(fit_note)),
    n_ok = sum(is.na(fit_note)),
